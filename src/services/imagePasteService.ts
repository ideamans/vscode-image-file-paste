...
export class ImagePasteService {
    async getImageFromClipboard(): Promise<Buffer | null> {
        ...
        try {
            if (platform === 'darwin') {
                ...
                try {
                    // まず画像があるかチェック
                    const { stdout: typeCheck } = await execAsync('osascript -e "clipboard info"');
    ...
    async getImageInfo(filePath: string): Promise<{ width: number; height: number }> {
    ...
    async saveImage(imageBuffer: Buffer, filePath: string, format: string): Promise<void> {
        const image = await Jimp.read(imageBuffer);
